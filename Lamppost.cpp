...
Lamppost::Lamppost() :position{ -1.0f, 4.5f, 0.0f , 1.0f },
target{ -5, 0.0f, 0.0f },
color{ 1.0f, 1.0f, 1.0f },
cutoff(30.0f),
exponent(0.0f) {}

void Lamppost::addlight() {
...
void Lamppost::draw() {
	...
	glPushMatrix();
	glTranslatef(0, 0, 0);
	glutSolidCylinder(0.3, 0.39, 10, 10);
	...
	glScalef(5, 0.5f * 0.3f, 0.5f * 0.3f);
	glutSolidCube(1);
	glPopMatrix();
...
void Lamppost::disable()
{
	glDisable(GL_LIGHT1);
}

void Lamppost::enable()
{
	glEnable(GL_LIGHT1);
}

void Lamppost::normalize(const GLfloat* vec, GLfloat* output)
{
...
void Lamppost::cross(const GLfloat* vec1, const GLfloat* vec2, GLfloat* output) {
	output[0] = vec1[1] * vec2[2] - vec1[2] * vec2[1];
	output[1] = vec1[2] * vec2[0] - vec1[0] * vec2[2];
	output[2] = vec1[0] * vec2[1] - vec1[1] * vec2[0];
}

void Lamppost::lookAt(GLfloat* eye, GLfloat* center, GLfloat* up)
{
